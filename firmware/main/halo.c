...
esp_err_t orb_halo_init(void){
    ...
    rmt_copy_encoder_config_t ec={0};e=rmt_new_copy_encoder(&ec,&encoder);if(e!=ESP_OK)return e;
...
esp_err_t orb_halo_rgb(uint8_t r,uint8_t g,uint8_t b){
    ...
    if(r>32)r=32;if(g>32)g=32;if(b>32)b=32;uint8_t rgb[3]={g,r,b};unsigned at=0;
    for(unsigned pixel=0;pixel<16;pixel++)for(unsigned c=0;c<3;c++)for(int bit=7;bit>=0;bit--){
        bool one=(rgb[c]>>bit)&1;symbols[at++]=(rmt_symbol_word_t){.level0=1,.duration0=one?8:4,.level1=0,.duration1=one?5:9};
    }
