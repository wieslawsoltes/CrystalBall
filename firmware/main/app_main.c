...
static void release(int pin){while(!gpio_get_level(pin))delay(20);delay(35);}
...
void app_main(void){
    ...
    for(;;){
        if(!orb_network_online()){
            if(!disconnected){(void)orb_display_status("OFFLINE","CHECK WIFI, DNS AND CLOCK. NO AUDIO IS RECORDED OR QUEUED.");disconnected=true;}
        ...
        if(pressed(PIN_PTT)){
            ...
            if(e==ESP_OK){
                ...
                if(orb_speech(text,&pcm,&pcm_bytes)==ESP_OK){(void)orb_audio_play(pcm,pcm_bytes);orb_wipe(pcm,pcm_bytes);free(pcm);release(PIN_PTT);}
#endif
            }else{
        ...
        if(erase_at&&esp_timer_get_time()>erase_at){orb_wipe(text,sizeof text);erase_at=0;(void)orb_display_status("READY","LAST RESPONSE ERASED. HOLD TALK TO ASK.");}
