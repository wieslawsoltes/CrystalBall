...
if(isPublic){
  ...
  function erase(){state.text='';state.page=0;state.eraseAt=0;$('reflection').hidden=true;$('answer').textContent='The last reflection has been erased. Ask a new question when you are ready.';$('answer-kind').textContent='A MOMENT OF STILLNESS';$('erase-count').textContent='';$('page-label').textContent='—';for(const id of ['previous','next','speak','erase'])$(id).disabled=true;if(!state.recording&&!state.voice)setActivity('idle')}
  async function cancel(){state.operation++;abort?.abort();api.cancel();recordDown=false;state.recording=false;state.voice=false;$('record').classList.remove('recording');$('record').textContent='● Hold to ask';$('live-voice').textContent='Live voice';$('level').style.width='0';speaker.stop();await Promise.allSettled([recorder.stop(false),live.stop()]);$('voice-note').textContent='Microphone off. Capture and playback stopped.';setActivity(state.text?'ready':'idle')}
  ...
  $('speak').addEventListener('click',async()=>{const op=++state.operation;abort=new AbortController();setActivity('speaking');$('voice-note').textContent='The voice you hear is AI-generated. Microphone remains off.';try{const pcm=await api.speech(state.text,AbortSignal.any([abort.signal,AbortSignal.timeout(60000)]));if(op!==state.operation){new Uint8Array(pcm).fill(0);return}await speaker.play(pcm,()=>{if(op===state.operation)setActivity('ready')})}catch(error){if(op===state.operation){setActivity('error');toast(error.message)}}});
